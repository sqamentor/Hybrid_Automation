...
class PageObjectGenerator:
    ...
    def generate_from_script(
        ...
        page_object_code = self._generate_page_class(page_name, locators, actions, url)
    ...
    def _extract_actions(self, code: str) -> List[Dict[str, Any]]:
        ...
        for action_type, pattern in action_patterns.items():
            matches = re.finditer(pattern, code)
            for match in matches:
                actions.append({"type": action_type, "code": match.group(0)})
        ...
        if locators:
            ...
            for locator in locators:
                ...
                if locator["type"] == "css":
                    ...
                    code_parts.append(
                        f'        return self.page.get_by_role("{locator["selector"]}")'
                    )
                elif locator["type"] == "text":
                    code_parts.append(
                        f'        return self.page.get_by_text("{locator["selector"]}")'
                    )
                elif locator["type"] == "testid":
                    code_parts.append(
                        f'        return self.page.get_by_test_id("{locator["selector"]}")'
                    )
                elif locator["type"] == "placeholder":
                    code_parts.append(
                        f'        return self.page.get_by_placeholder("{locator["selector"]}")'
                    )
                elif locator["type"] == "label":
                    code_parts.append(
                        f'        return self.page.get_by_label("{locator["selector"]}")'
                    )
    ...
    def list_page_objects(self) -> List[Dict[str, Any]]:
        ...
        for py_file in self.output_dir.glob("*.py"):
            if py_file.name != "__init__.py":
                page_objects.append(
                    {"name": py_file.stem, "path": str(py_file), "size": py_file.stat().st_size}
                )
