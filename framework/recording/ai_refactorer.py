...
class AIScriptRefactorer:
    ...
    def _initialize_ai(self):
        ...
        try:
            ...
            if self.ai_provider:
                self.enabled = True
                logger.info(f"AI Refactorer initialized with {self.ai_provider.get_provider_name()}")
            else:
                logger.warning("AI provider not available. Refactorer will use rule-based improvements only.")
                self.enabled = False
