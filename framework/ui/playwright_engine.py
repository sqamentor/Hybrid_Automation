...
class PlaywrightEngine:
    ...
    def start(self, browser_type: str = "chromium"):
        ...
        for attempt in range(self.max_retries):
            try:
                logger.info(
                    f"Starting Playwright {browser_type} (attempt {attempt + 1}/{self.max_retries})"
                )
                ...
                if self.enable_context_pool:
                    self.context_pool = ContextPool(
                        browser=self.browser, pool_size=self.pool_size, headless=self.headless
                    )
                ...
                logger.info(
                    f"Playwright {browser_type} started successfully (headless={self.headless})"
                )
                return  # Success!
