...
class UIFactory:
    ...
    def create_engine(
        self, test_metadata: dict, browser_type: str = "chromium", headless: bool = True
        ...
        if self.ai_selector:
            ...
            if decision:
                logger.info(
                    f"AI selected engine: {decision.engine} (confidence: {decision.confidence}%)"
                )
    ...
    def execute_with_fallback(
        self, test_func, test_metadata: dict, browser_type: str = "chromium", headless: bool = True
    ) -> dict:
        """
        ...
        """
        result = {"attempts": [], "final_status": "unknown", "fallback_triggered": False}
        ...
        try:
            ...
            result["attempts"].append(
                {"engine": decision.engine, "status": "passed", "error": None}
            )
            ...
            should_fallback = (
                decision.engine == "playwright" and self.engine_selector.should_fallback(error_type)
            )
            ...
            if should_fallback:
                ...
                try:
                    ...
                    result["attempts"].append(
                        {"engine": "selenium", "status": "passed", "error": None}
                    )
                    ...
                    result["attempts"].append(
                        {"engine": "selenium", "status": "failed", "error": str(fallback_error)}
                    )
