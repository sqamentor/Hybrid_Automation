...
class OpenAIProvider(BaseAIProvider):
    ...
    def _initialize(self):
        ...
        try:
            ...
            logger.info(
                f"OpenAI provider '{self.config.name}' initialized with model {self.config.model}"
            )
...
class AnthropicProvider(BaseAIProvider):
    ...
    def _initialize(self):
        ...
        try:
            ...
            if not api_key:
                logger.warning(
                    f"Anthropic API key not found. Provider '{self.config.name}' disabled."
                )
                self.config.enabled = False
                return
            ...
            logger.info(
                f"Anthropic provider '{self.config.name}' initialized with model {self.config.model}"
            )
    ...
    def _initialize(self):
        ...
        try:
            ...
            if not api_key or not azure_endpoint:
                logger.warning(
                    f"Azure OpenAI credentials not found. Provider '{self.config.name}' disabled."
                )
                self.config.enabled = False
                return
...
class OllamaProvider(BaseAIProvider):
    ...
    def _initialize(self):
        ...
        try:
            ...
            self.api_base = self.config.api_base or os.getenv(
            ...
            try:
                ...
                if response.status_code == 200:
                    logger.info(
                        f"Ollama provider '{self.config.name}' initialized with model {self.config.model}"
                    )
                    self.client = True  # Just a flag
                else:
                    logger.warning(
                        f"Ollama not responding. Provider '{self.config.name}' disabled."
                    )
                    self.config.enabled = False
            except Exception:
                logger.warning(
                    f"Ollama not running at {self.api_base}. Provider '{self.config.name}' disabled."
                )
                self.config.enabled = False
...
class AIProviderFactory:
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            "default": "openai",
            "providers": {
                "openai": {"type": "openai", "model": "gpt-4", "enabled": True, "priority": 1},
                "claude": {
                    "type": "anthropic",
                ...
                "azure": {"type": "azure", "enabled": False, "priority": 3},
                "ollama": {"type": "ollama", "model": "llama3.1", "enabled": False, "priority": 4},
            },
        }
    ...
    def get_provider(self, provider_name: Optional[str] = None) -> Optional[BaseAIProvider]:
        ...
        # Sort by priority
        sorted_providers = sorted(self._providers.items(), key=lambda x: x[1].config.priority)
