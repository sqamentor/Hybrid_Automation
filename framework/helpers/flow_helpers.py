...
def run_api_validation(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs) -> Optional[Any]:
        if should_run_api_validation():
            ...
            logger.info(
                f"Skipping API validation: {func.__name__} (execution mode: {get_execution_mode()})"
            )
            return None
...
def run_db_validation(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs) -> Optional[Any]:
        if should_run_db_validation():
            ...
            logger.info(
                f"Skipping database validation: {func.__name__} (execution mode: {get_execution_mode()})"
            )
            return None
