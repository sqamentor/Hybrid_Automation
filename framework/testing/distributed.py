...
def pytest_configure(config):
    ...
    """
    config.addinivalue_line("markers", "distributed: mark test to run in distributed mode only")
    config.addinivalue_line("markers", "group(name): assign test to a distribution group")
...
class LoadBalancer:
    ...
    def balance_by_duration(test_items: list, num_workers: int) -> Dict[int, list]:
        ...
        sorted_items = sorted(
            test_items, key=lambda item: getattr(item, "estimated_duration", 1.0), reverse=True
        )
    ...
    def balance_by_complexity(test_items: list, num_workers: int) -> Dict[int, list]:
        ...
        sorted_items = sorted(
            test_items, key=lambda item: getattr(item, "complexity_score", 1), reverse=True
        )
...
class DistributedResultAggregator:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        summary["total_tests"] = (
            summary["total_passed"] + summary["total_failed"] + summary["total_skipped"]
        )
    ...
    def get_worker_statistics(self) -> Dict[str, Dict[str, Any]]:
        ...
        for worker_id, result in self.worker_results.items():
            total = result.get("passed", 0) + result.get("failed", 0) + result.get("skipped", 0)
