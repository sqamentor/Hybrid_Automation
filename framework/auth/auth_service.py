...
class AuthenticationService:
    ...
    def authenticate_sso(
        self, engine, sso_config: Dict[str, str], credentials: Dict[str, str], timeout: int = 60
    ...
    def _authenticate_okta(
        self, driver, sso_config: Dict[str, str], credentials: Dict[str, str], timeout: int
    ) -> Optional[SessionData]:
        ...
        try:
            ...
            if mfa_token:
                ...
                try:
                    mfa_field = wait.until(EC.presence_of_element_located((By.NAME, "answer")))
                    mfa_field.send_keys(mfa_token)
                    mfa_submit = driver.find_element(
            ...
            self._current_user = {"username": username, "auth_type": "SSO", "provider": "okta"}
    ...
    def _authenticate_azure_ad(
        ...
        try:
            ...
            if mfa_token:
                self.logger.info("MFA required - entering MFA token...")
                try:
                    mfa_field = wait.until(EC.presence_of_element_located((By.NAME, "otc")))
                    mfa_field.send_keys(mfa_token)
            ...
            session_data = self.session_manager.extract_session_from_selenium(driver)
            self._current_session = session_data
            self._current_user = {"username": username, "auth_type": "SSO", "provider": "azure_ad"}
    ...
    def _authenticate_google(
        ...
        try:
            ...
            session_data = self.session_manager.extract_session_from_selenium(driver)
            self._current_session = session_data
            self._current_user = {"username": username, "auth_type": "SSO", "provider": "google"}
    ...
    def authenticate_basic(
        self, engine, username: str, password: str, login_url: str, timeout: int = 30
    ) -> Optional[SessionData]:
        """
        ...
        try:
            ...
            if hasattr(engine, "get"):  # Selenium
                return self._authenticate_basic_selenium(
                    engine, username, password, login_url, timeout
                )
            else:  # Playwright
                return self._authenticate_basic_playwright(
                    engine, username, password, login_url, timeout
                )
    ...
    def _authenticate_basic_selenium(
        self, driver, username: str, password: str, login_url: str, timeout: int
    ) -> Optional[SessionData]:
    ...
    def _authenticate_basic_playwright(
        self, page, username: str, password: str, login_url: str, timeout: int
    ) -> Optional[SessionData]:
    ...
    async def switch_engine_with_session(
        self, from_engine, to_engine_page, from_type: str = "selenium", to_type: str = "playwright"
    ) -> bool:
        """
