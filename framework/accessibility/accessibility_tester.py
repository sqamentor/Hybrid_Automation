...
class AccessibilityTester:
    ...
    def _analyze_with_playwright(self, options: Optional[Dict] = None) -> Dict:
        ...
        axe_options.setdefault(
            "runOnly", {"type": "tag", "values": [self.wcag_level.value, "best-practice"]}
        )
    ...
    def _log_summary(self, results: Dict):
        ...
        if violations:
            for violation in violations:
                ...
                logger.warning(
                    f"  [{impact}] {violation.get('id')}: {violation.get('description')}"
                )
    ...
    def get_violations(self, min_impact: Optional[ImpactLevel] = None) -> List[Dict]:
        ...
        return [
            v for v in self.violations if impact_order.get(v.get("impact", "minor"), 0) >= min_level
        ]
    ...
    def check_color_contrast(self) -> Dict[str, Any]:
        ...
        contrast_violations = [v for v in self.violations if "color-contrast" in v.get("id", "")]

        return {"passed": len(contrast_violations) == 0, "violations": contrast_violations}
    ...
    def check_alt_text(self) -> Dict[str, Any]:
        ...
        """
        alt_violations = [v for v in self.violations if "image-alt" in v.get("id", "")]

        return {"passed": len(alt_violations) == 0, "violations": alt_violations}
    ...
    def check_aria_labels(self) -> Dict[str, Any]:
        ...
        """
        aria_violations = [v for v in self.violations if "aria" in v.get("id", "").lower()]

        return {"passed": len(aria_violations) == 0, "violations": aria_violations}
