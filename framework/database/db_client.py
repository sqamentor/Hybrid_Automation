...
class DBClient:
    ...
    def connect(self):
        ...
        audit_logger.log_action(
            "db_connect",
            {"database": self.config.name, "host": self.config.host, "port": self.config.port},
        )
    ...
    def execute_query(self, query: str, params: Optional[Dict] = None) -> List[Dict]:
        ...
        if not self._is_read_only_query(query):
            ...
            audit_logger.log_error(error_type="db_query_blocked", error_message=error_msg)
    ...
    def close(self):
        ...
        if self.engine:
            ...
            audit_logger.log_action("db_disconnect", {"database": self.config.name})
