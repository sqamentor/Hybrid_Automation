...
class PluginManager:
    ...
    def disable_plugin(self, plugin_name: str) -> bool:
        ...
        try:
            ...
            for hook_name, hooks in self._hooks.items():
                self._hooks[hook_name] = [h for h in hooks if h.plugin_name != plugin_name]
    ...
    def register_hook(self, hook: PluginHookData) -> None:
        ...
        self._hooks[hook.name].sort(
            key=lambda h: (
                h.priority.value if isinstance(h.priority, PluginPriority) else h.priority
            ),
            reverse=True,
        )
