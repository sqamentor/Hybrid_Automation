...
def main(cli_args: Optional[List[str]] = None):
    """Entry point that can accept args from unified CLI"""
    # If called from unified CLI with args, use those; otherwise use sys.argv
    if cli_args is not None:
        original_argv = sys.argv
        sys.argv = ['automation record'] + cli_args
        try:
            return _main()
        finally:
