...
class BrowserConfig(BaseModel):
    ...
    args: List[str] = Field(
        default_factory=lambda: ["--start-maximized"],
        description="Browser launch arguments"
    )
    downloads_path: Optional[str] = Field(
        default=None,
        description="Custom downloads directory path"
    )
    trace_on_first_retry: bool = Field(
        default=False,
        description="Enable trace on first retry"
    )
    ...
    video_on_failure: bool = Field(
        default=False,
        description="Record video on test failure"
    )
...
class APIConfig(BaseModel):
    ...
    retry_attempts: int = Field(default=3, ge=0, le=10, description="Number of retry attempts", alias="retry_count")
...
class EnvironmentConfig(BaseModel):
    ...
    databases: Dict[str, DatabaseConfig] = Field(default_factory=dict, description="Database configs")
...
class EngineDecisionMatrix(BaseModel):
    ...
    requires_javascript: bool = Field(
        default=True,
        description="Does the test require JavaScript execution"
    )
    is_legacy_ui: bool = Field(
        default=False,
        description="Is this a legacy UI (old HTML/CSS)"
    )
    requires_mobile_testing: bool = Field(
        default=False,
        description="Does this require mobile testing"
    )
    test_complexity: str = Field(
        default="medium",
        description="Test complexity level (low/medium/high)"
    )
    
    # Legacy support for rules-based approach
    rules: List[Dict[str, Any]] = Field(
        default_factory=list,
        description="Optional custom decision rules"
    )
    default_engine: EngineType = Field(
        default=EngineType.PLAYWRIGHT,
        description="Default engine if no rules match"
    )
...
class FrameworkConfig(BaseSettings):
    ...
    config_dir: Path = Field(
        default_factory=lambda: Path.cwd() / "config",
    ...
    environment: TestEnvironment = Field(
        default=TestEnvironment.DEV,
        description="Current test environment"
    )
    ...
    video_dir: Path = Field(
        default_factory=lambda: Path.cwd() / "videos",
        description="Video recording directory"
    )
