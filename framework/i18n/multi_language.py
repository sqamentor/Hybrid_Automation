...
class MultiLanguageSupport:
    ...
    def get_all_translations(self, key: str) -> Dict[str, str]:
        ...
        """
        return {lang: self.translations[lang].get(key, key) for lang in self.translations}
    ...
    def set_browser_language(self, ui_engine, lang_code: str):
        ...
        if engine_type == "PlaywrightEngine":
            ...
            logger.warning(
                "Browser language must be set during browser initialization for Selenium"
            )

    def verify_translations_exist(
        self, keys: List[str], languages: Optional[List[str]] = None
    ) -> Dict[str, List[str]]:
        """
    ...
    def test_text_rendering(
        self, ui_engine, element_locator: str, expected_key: str
    ) -> Dict[str, bool]:
        """
    ...
    def generate_sample_translations(self, output_dir: Optional[str] = None):
        ...
        for lang_code, lang_info in self.LANGUAGES.items():
            ...
            lang_translations = {
                key: f"{value} [{lang_code.upper()}]" for key, value in sample_translations.items()
            }
