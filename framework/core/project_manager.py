...
class ProjectManager:
    ...
    def detect_environment_from_url(self, url: str, project: str) -> str:
        ...
        if env_from_var and self.global_settings.get("environment_detection", {}).get(
            "prefer_env_variable", True
        ):
        ...
        if self.global_settings.get("environment_detection", {}).get("auto_detect", True):
            ...
            staging_keywords = self.global_settings.get("environment_detection", {}).get(
    ...
    def get_project_config(self, project: str) -> Dict[str, Any]:
        ...
        if project not in self.projects:
            raise ProjectConfigError(
                f"Project '{project}' not found. " f"Available: {list(self.projects.keys())}"
            )
    ...
    def get_project_paths(self, project: str) -> Dict[str, Path]:
        ...
        return {
            ...
            "recorded_tests": self.workspace_root
            / paths.get("recorded_tests", f"recorded_tests/{project}"),
    ...
    def generate_recording_filename(
        ...
        # Build filename
        filename = pattern.format(project=project, feature=feature, timestamp=ts_string)
