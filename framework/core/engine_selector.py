...
class EngineSelector:
    ...
    def _select_engine_internal(self, test_metadata: Dict[str, Any]) -> EngineDecision:
        ...
        for rule in self.rules:
            if self._evaluate_rule(rule, test_metadata):
                ...
                priority = (
                    priority_override if priority_override is not None else rule.get("priority", 50)
                )
    ...
    def _cache_decision(
        self, cache_key: str, decision: EngineDecision, test_metadata: Dict[str, Any]
    ):
        """
        ...
        # Create cache entry
        entry = CacheEntry(
            decision=decision, timestamp=time.time(), hit_count=0, metadata_hash=cache_key
        )
    ...
    def _get_top_cached_patterns(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        sorted_entries = sorted(
            self._decision_cache.items(), key=lambda x: x[1].hit_count, reverse=True
        )[:limit]
...
def extract_test_metadata(test_item) -> Dict[str, Any]:
    ...
    if (
        "React" in str(metadata["ui_framework"])
        or "Vue" in str(metadata["ui_framework"])
        or "Angular" in str(metadata["ui_framework"])
    ):
