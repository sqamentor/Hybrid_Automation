...
class AIEngineSelector:
    ...
    def __init__(
        ...
        self._cache_stats = {"hits": 0, "misses": 0, "evictions": 0, "total_lookups": 0}
    ...
    def select_engine(
        ...
        if not self.enabled or not self.client:
            logger.warning(
                "AI helper not available for engine selection. Skipping AI-based engine recommendation. Continuing with default engine."
            )
            return None
        ...
        try:
            ...
            if response:
                ...
                if decision:
                    ...
                    logger.info(
                        f"AI recommendation for {test_metadata.get('test_name')}: {decision.engine} ({response_time:.0f}ms)"
                    )
                return decision
    ...
    def _build_prompt(
        self, test_metadata: Dict[str, Any], historical_data: Optional[Dict[str, Any]]
    ) -> str:
        ...
        prompt += "\n\nCapability Summary:\n"
        prompt += (
            "Playwright: Best for modern SPA, API interception, fast execution, mobile emulation\n"
        )
        prompt += (
            "Selenium: Best for legacy UI, SSO/MFA, deep iframes, long sessions, enterprise auth\n"
        )
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt < self.max_retries - 1:
                    ...
                    logger.warning(
                        f"AI query attempt {attempt + 1} failed: {e}. Retrying in {delay}s..."
                    )
                    time.sleep(delay)
                else:
            ...
            if self.provider_type in ["openai", "azure_openai"]:
                response = self.client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": "You are a QA automation expert who selects the optimal UI testing engine.",
                        },
    ...
    def _query_llamacpp(self, prompt: str) -> Optional[str]:
        ...
        try:
            ...
            system_prompt = (
                "You are a QA automation expert who selects the optimal UI testing engine."
            )
            full_prompt = f"{system_prompt}\n\n{prompt}"

            response = llm(
                full_prompt, max_tokens=500, temperature=0.3, stop=["</s>", "\n\n\n"], echo=False
            )
    ...
    def analyze_test_suite(self, test_items: list) -> Dict[str, Any]:
        ...
        if not self.enabled:
            logger.warning(
                "AI helper not available for batch test analysis. Skipping AI analysis. Continuing without recommendations."
            )
            return {
                "error": "AI not enabled",
                "message": "AI helper not available, skipping this step",
            }
    ...
    def _cache_response(self, cache_key: str, response: str):
        ...
        entry = CachedResponse(
            response=response, timestamp=time.time(), hit_count=0, metadata_hash=cache_key
        )
