    ...
    def _execute_step_sync(
        ...
        try:
            ...
            if step.requires_session and workflow.session_data:
                ...
                if step.engine_type == EngineType.SELENIUM:
                    # Inject to Selenium
                    success = self.session_manager.inject_session_to_selenium(engine, workflow.session_data)
                    if not success:
                        raise ValueError("Failed to inject session to Selenium")
                
                elif step.engine_type == EngineType.PLAYWRIGHT:
                    # Inject to Playwright (sync)
                    success = self.session_manager.inject_session_to_playwright_sync(engine, workflow.session_data)
                    if not success:
        ...
        try:
            ...
            if step.requires_session and workflow.session_data:
                ...
                if step.engine_type == EngineType.PLAYWRIGHT_ASYNC:
                    success = await self.session_manager.inject_session_to_playwright(engine, workflow.session_data)
                    if not success:
