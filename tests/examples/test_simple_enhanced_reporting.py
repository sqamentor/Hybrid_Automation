...
def test_with_api_calls(report_log, report_api_call):
    ...
    report_api_call(
        method="GET", url="https://api.example.com/users", status_code=200, response_time=125.5
    )
...
def test_with_database(report_log, report_db_query):
    ...
    report_db_query(
        query="SELECT * FROM users WHERE active = 1", execution_time=45.2, rows_affected=10
    )
...
def test_comprehensive(report_log, report_api_call, report_db_query, report_assertion):
    ...
    report_api_call(
        method="GET", url="https://api.example.com/health", status_code=200, response_time=50.0
    )

    # Phase 2: Database
    report_log("Phase 2: Database Testing", "INFO")
    report_db_query(query="SELECT COUNT(*) FROM users", execution_time=25.0, rows_affected=1)
