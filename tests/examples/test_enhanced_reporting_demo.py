...
@pytest.mark.critical
@pytest.mark.slow
def test_comprehensive_enhanced_report(
    report_log, report_api_call, report_db_query, report_assertion
):
    """
    ...
    report_db_query(
        query="SELECT * FROM users WHERE id = 456", execution_time=23.4, rows_affected=1
    )
    ...
    report_db_query(query="DELETE FROM users WHERE id = 456", execution_time=15.2, rows_affected=1)
