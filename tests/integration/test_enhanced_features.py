...
class TestEnhancedIntegration:
    ...
    def test_complete_flow_with_all_enhancements(
        self, ui_engine, api_interceptor, ai_validator, db_client, test_context
    ):
        """
        ...
        for suggestion in suggestions:
            if suggestion["confidence_score"] >= 85:
                ...
                print(
                    f"  • {suggestion['validation_type']}: {suggestion['reason']} "
    ...
    def test_request_modification_feature(self, api_interceptor):
        ...
        modified_request = modifier.apply(
            {"method": "POST", "url": "/api/appointments", "headers": {}, "body": original_body}
        )
    ...
    def test_response_mocking_feature(self, api_interceptor):
        ...
        original_response = {"status_code": 200, "body": {"appointmentId": 123}, "headers": {}}
    ...
    def test_confidence_scoring(self, ai_validator):
        ...
        high_confidence_suggestions = ai_validator.suggest_validations(
            ...
            response_data={"success": True, "orderId": 123, "status": "Confirmed"},
        )

        # Check for high confidence scores
        high_confidence_count = sum(
            1 for s in high_confidence_suggestions if s["confidence_score"] >= 85
        )
        ...
        for suggestion in high_confidence_suggestions[:3]:
            print(
                f"  • {suggestion['validation_type']}: {suggestion['confidence_score']}% confidence"
            )
