...
class TestAIDrivenValidation:
    ...
    def test_ai_validation_with_auto_execute(self, api_client, db_client):
        ...
        critical_validations = [
            v for v in results["validations_executed"] if "passed" in v and v["passed"]
        ]
    ...
    def test_ai_learns_from_mapping(self, api_client, db_client):
        ...
        assert any(
            "inventory" in reason for reason in validation_reasons
        ), "Should suggest inventory validation"
        assert any(
            "audit" in reason or "log" in reason for reason in validation_reasons
...
class TestUserRegistrationValidation:
    ...
    def test_user_registration_validations(self, api_client, db_client):
        ...
        critical_suggestions = [s for s in strategy.suggestions if s.priority == "critical"]
...
def test_ai_fallback_when_disabled(api_client):
    ...
    try:
        ...
        assert "fallback" in strategy.ai_reasoning.lower(), "Should indicate fallback was used"
...
def test_validation_suggestion_performance(api_client):
    ...
    response = api_client.post(
        "/api/orders/submit",
        json_data={"customer_id": "TEST", "items": [{"product_id": "P1", "quantity": 1}]},
    )
