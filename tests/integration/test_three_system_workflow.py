...
class TestThreeSystemWorkflow:
    ...
    def test_book_and_verify_in_all_systems(
        self, test_context: TestContext, bookslot_page, patientintake_page, callcenter_page
    ):
        """
        ...
        bookslot_page.navigate().fill_patient_info(appointment).submit()
        ...
        patientintake_page.navigate().search_appointment(appointment).verify_appointment_exists(
            appointment
        )
        ...
        callcenter_page.navigate().search_appointment(appointment).verify_appointment_exists(
            appointment
        )
    ...
    def test_cancel_from_callcenter_only(
        ...
        callcenter_page.cancel_appointment(email=appointment.email, reason=cancellation_reason)
        ...
        patientintake_page.navigate().search_appointment(appointment)
        assert patientintake_page.should_have_status(
            appointment.email, "Cancelled"
    ...
    def test_callcenter_exclusive_cancellation_permission(
        ...
        callcenter_page.navigate().search_appointment(appointment)
        ...
        callcenter_page.cancel_appointment(
            email=appointment.email, reason="Testing exclusive cancellation permission"
        )
    ...
    def test_multi_appointment_workflow(
        ...
        for appointment in appointments:
            patientintake_page.navigate().search_appointment(appointment).should_show_appointment(
                appointment
            )
        ...
        for appointment in appointments:
            callcenter_page.navigate().search_appointment(appointment).should_show_appointment(
                appointment
            )
        ...
        callcenter_page.navigate().search_appointment(appointment_to_cancel).cancel_appointment(
            email=appointment_to_cancel.email, reason="Patient rescheduled"
        )

        # Verify mixed statuses
        logger.info("Verifying mixed statuses (cancelled + active)...")
        callcenter_page.navigate().search_appointment(appointment_to_cancel)
        assert callcenter_page.should_have_status(
            appointment_to_cancel.email, "Cancelled"
        ), f"Expected cancelled appointment for {appointment_to_cancel.email}"

        callcenter_page.navigate().search_appointment(appointments[1])
        assert callcenter_page.should_have_status(
            appointments[1].email, "Active"
...
class TestThreeSystemSmoke:
    ...
    def test_all_systems_accessible(self, bookslot_page, patientintake_page, callcenter_page):
        """
