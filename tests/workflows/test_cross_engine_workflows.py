...
def test_sso_to_callcenter_to_patientintake(
    workflow_orchestrator, workflow_engines, auth_service, sso_config, sso_credentials, config
):
    """
    ...
    callcenter_url = (
        config.get("projects", {})
        .get("callcenter", {})
        .get("url", "https://callcenter.example.com")
    )
    patientintake_url = (
        config.get("projects", {})
        .get("patientintake", {})
        .get("url", "https://patientintake.example.com")
    )
    ...
    def step1_sso_login(driver):
        ...
        session_data = auth_service.authenticate_sso(
            engine=driver, sso_config=sso_config, credentials=sso_credentials, timeout=60
        )
...
def test_sso_authentication_only(
    workflow_orchestrator, workflow_engines, auth_service, sso_config, sso_credentials
):
    """
...
def test_session_transfer_selenium_to_playwright(
    ...
    def verify_playwright_session(page):
        # Get any project URL
        test_url = (
            config.get("projects", {}).get("callcenter", {}).get("url", "https://example.com")
        )
