...
class TestBookslotCompleteFlows:
    ...
    def test_complete_booking_new_patient_morning(
        ...
        with allure.step("Step 6: Fill insurance information"):
            act.type_text(page.get_by_role("textbox", name="Member Name *"), data['MemberName'], "Member")
            act.type_text(page.get_by_role("textbox", name="ID Number *"), data['idNumber'], "ID")
            act.type_text(page.get_by_role("textbox", name="Group Number"), data['GroupNumber'], "Group")
            act.type_text(page.get_by_role("textbox", name="Payer Name *"), data['PayerName'], "Payer")
            act.button_click(page.get_by_role("button", name="Next"), "Next")
        
        with allure.step("Step 7: Verify booking success"):
            page.wait_for_timeout(1000)
            assert "/success" in page.url.lower() or "confirmation" in page.url.lower()
            allure.attach(page.screenshot(full_page=True), name="booking_success", 
    ...
    @pytest.mark.parametrize("insurance_provider", [
    ...
    def test_booking_with_various_insurance_providers(
        self, page: Page, multi_project_config, smart_actions, fake_bookslot_data, insurance_provider
    ):
        """
        ...
        with allure.step(f"Book appointment with {insurance_provider}"):
            ...
            act.type_text(page.get_by_role("textbox", name="Payer Name *"), insurance_provider, "Payer")
            act.button_click(page.get_by_role("button", name="Next"), "Next")
    ...
    @pytest.mark.parametrize("referral_source", [
        "Referred by physician", "Online search", "Social media", "Friend or family", "Advertisement"
    ...
    def test_booking_with_human_behavior_simulation(
        ...
        with allure.step("Complete booking with human-like behavior"):
            ...
            # Verify success
            assert "/success" in page.url.lower() or "confirmation" in page.url.lower() or "thank" in page.url.lower()
            
            print(f"\n✅ Test completed successfully with {data['email']}\n")
            
            allure.attach(page.screenshot(full_page=True), name="booking_success_complete", 
                         attachment_type=allure.attachment_type.PNG)
            allure.attach("Human behavior simulation completed with ALL recorded test features", 
                         name="behavior_test", 
    ...
    def test_booking_with_error_recovery(
        ...
        with allure.step("Attempt with invalid email"):
            ...
            act.type_text(page.get_by_role("textbox", name="First Name *"), data['first_name'], "First Name")
            act.type_text(page.get_by_role("textbox", name="Last Name *"), data['last_name'], "Last Name")
            act.type_text(page.get_by_role("textbox", name="Email *"), "invalid.email", "Invalid Email")
            act.type_text(page.get_by_role("textbox", name="Phone *"), data['phone_number'], "Phone")
            act.button_click(page.get_by_role("button", name="Next"), "Next")
        ...
        with allure.step("Correct email and complete booking"):
            ...
            act.type_text(page.get_by_role("textbox", name="Date of Birth *"), data['dob'], "DOB")
            act.type_text(page.get_by_role("textbox", name="Address *"), "777 Recovery St", "Address")
            ...
            assert "/success" in page.url.lower() or "confirmation" in page.url.lower()
            allure.attach("User successfully recovered from error", name="error_recovery", 
                         attachment_type=allure.attachment_type.TEXT)
    ...
    def test_data_persistence_during_navigation(
        ...
        with allure.step("Navigate back and verify data persistence"):
            ...
            allure.attach(page.screenshot(full_page=True), name="data_persistence_check", 
                         attachment_type=allure.attachment_type.PNG)
