...
class TestBasicInfoPage:
    ...
    def test_basic_info_page_loads(self, smart_actions, fake_bookslot_data, multi_project_config):
        ...
        with allure.step("Verify form fields are visible"):
            ...
            allure.attach(basic_info_page.page.screenshot(full_page=True), 
    ...
    ], ids=["valid_simple", "valid_subdomain", "valid_plus", "no_at", "no_prefix", "no_domain", "spaces", "double_at"])
    def test_email_validation(self, smart_actions, fake_bookslot_data, multi_project_config, email, is_valid):
        """
        ...
        with allure.step(f"Test email: {email} (Expected valid: {is_valid})"):
            ...
            assert email in email_value or email == email_value, f"Email field should contain {email}"
    ...
    @pytest.mark.parametrize("phone,is_valid", [
        ...
        ("abcdefghij", False),
        ("555-abc-defg", False),
    ], ids=["plain_10", "alt_10", "formatted_parens", "formatted_dash", "international", "too_short_3", "too_short_5", "letters", "mixed"])
    def test_phone_validation(self, smart_actions, fake_bookslot_data, multi_project_config, phone, is_valid):
        """
    ...
    def test_required_fields(self, smart_actions, fake_bookslot_data, multi_project_config):
        ...
        with allure.step("Attempt submission with empty form"):
            ...
            assert not basic_info_page.is_next_button_visible(), "Next button should not be visible without required fields"
            
            # Verify page didn't progress
            assert basic_info_page.page.url == current_url, "Should not proceed without required fields"
            allure.attach(basic_info_page.page.screenshot(full_page=True), 
    ...
    def test_successful_submission(self, smart_actions, fake_bookslot_data, multi_project_config):
        ...
        with allure.step("Submit for OTP"):
            ...
            assert basic_info_page.is_next_button_visible(), "Next button should be visible after OTP verification"
            ...
            assert current_url_after != current_url_before, "Should proceed to next page after complete form submission"
            allure.attach(basic_info_page.page.screenshot(full_page=True), 
    ...
    @allure.severity(allure.severity_level.NORMAL)
    @pytest.mark.validation
    def test_special_characters_in_names(self, smart_actions, fake_bookslot_data, multi_project_config):
        """
        ...
        with allure.step("Test special characters"):
            ...
            assert "Mary-Anne" in first_value or "Mary" in first_value, "First name with hyphen should be accepted"
            assert "O'Brien" in last_value or "Brien" in last_value, "Last name with apostrophe should be accepted"
