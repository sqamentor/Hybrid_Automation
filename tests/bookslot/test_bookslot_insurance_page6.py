...
class TestInsurancePage:
    ...
    def test_insurance_page_loads(self, smart_actions, fake_bookslot_data, multi_project_config):
        ...
        insurance_page = BookslotInsurancePage(smart_actions.page, multi_project_config['bookslot']['ui_url'])
    ...
    @pytest.mark.validation
    @pytest.mark.parametrize("id_number,is_valid", [
        ("123456789", True),
        ("ABC123456", True),
        ("ABC123XYZ", True),
        ("12345678901234", True),
        ...
        ("12", False),
        ("", False),
    ], ids=["numeric", "alpha_numeric", "mixed", "long_id", "formatted", "too_short_3", "too_short_2", "empty"])
    def test_insurance_id_validation(self, smart_actions, fake_bookslot_data, id_number, is_valid):
        ...
        with allure.step(f"Test insurance ID: {id_number} (Expected valid: {is_valid})"):
            ...
            if is_valid:
                assert smart_actions.page.url != initial_url, f"Valid ID {id_number} should allow progression"
            else:
                assert smart_actions.page.url == initial_url, f"Invalid ID {id_number} should prevent progression"
    ...
    def test_group_number_optional(self, smart_actions, fake_bookslot_data, multi_project_config):
        ...
        with allure.step("Submit without group number"):
            ...
            assert smart_actions.page.url != initial_url, "Should allow progression without group number"
    ...
    @allure.severity(allure.severity_level.NORMAL)
    @pytest.mark.validation
    def test_group_number_accepts_input(self, smart_actions, fake_bookslot_data, multi_project_config):
        """
    ...
    @allure.severity(allure.severity_level.NORMAL)
    @pytest.mark.regression
    def test_major_insurance_providers(self, smart_actions, fake_bookslot_data, multi_project_config):
        """
        ...
        with allure.step("Verify major providers are accepted"):
            ...
            allure.attach(f"Major providers tested: {len(major_providers)}", 
